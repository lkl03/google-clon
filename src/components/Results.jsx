          ...
          {results?.map(({link, title, description}, index) => (
            <div key={index} className='md:w-2/5 w-full'>
              <a href={link} target="_blank" rel="noreferrer">
                <p className='text-sm'>
                  {link.length > 31 ? `${link.substring(0, 31)}(...)` : link}
                </p>
          ...
          {results?.map(({links, title, id, source, published}) => (
            <div key={id} className='md:w-2/5 w-full'>
              ...
              <div className='flex gap-4'>
                <a href={source?.href} target="_blank" rel="noreferrer" className='font-semibold'>
                  {source?.title}
                </a>
              </div>
              <p className='text-xs dark:text-green-200 text-slate-900'>
                {published}
              </p>
            </div>
          ))}
